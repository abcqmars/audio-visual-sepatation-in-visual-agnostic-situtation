...
class Resize(object):
    def __init__(self, size, interpolation=Image.BILINEAR, max_size=448):
        self.size = size
    ...
    def __call__(self, frames):
        ...
        out_frames = []
        for frame in frames:
            out_frames.append(F.resize(frame, self.size, self.interpolation, max_size=self.max_size))
        return out_frames

class PadImg(object):
    def __init__(self, pad_value, pad_size, pad_type="constant"):
    ...
    def __call__(self, frames):
        ...
        for frame in frames:
            print(frame.size)
            out_frames.append(F.pad(frame, padding=self.pad_size, fill=self.pad_value, padding_mode=self.pad_type))
        return out_frames
