...
class ResnetDilated(nn.Module):
    def __init__(self, orig_resnet, fc_dim=64, pool_type='maxpool',
        ...
        # self.fc.weight.register_hook(grad_checking_hook)
    ...
    def forward_multiframe(self, x, pool=True):
        ...
        if not pool:
            return torch.mean(x, dim=2) # B, C, H, W
...
def grad_checking_hook(grad):
    print("check the grad information of the fc.")
    print(grad)
    torch.save(grad, "/mnt/data2/he/src/Conv_TasNet_visual/debug/grad.pt")
    return grad